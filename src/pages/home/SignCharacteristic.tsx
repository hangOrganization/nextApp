...
export default function SignCharacteristic({ innerWidth, setRight, right, characteristicType, setCharacteristicType }: SignCharacteristicProps) {
    console.log("🚀 ~ file: SignCharacteristic.tsx:89 ~ SignCharacteristic ~ right:", right)
    const swiper = useSwiper()
    const dispatch = useAppDispatch()
    const throttleFlag = useThrottleFlag()
    const [mobileRight, setMobileRight] = useState<number>(0)
    return (
        <Box className="flex w-screen max-md:pb-[210px] md:overflow-hidden">
            <div className='flex'>
                <SignBox className={`md:flex ${right === 0 ? 'WhySIGN' : 'ABOUT'} md:h-[1278px] max-md:pt-[300px] relative w-screen items-center md:px-[120px] justify-between`}
                    onWheel={
                        (e: any) => {
                            console.log("🚀 ~ file: SignCharacteristic.tsx:165 ~ SignCharacteristic ~ e:", e.deltaY)
                ...
                >
                    ...
                    <div className={`transition-all duration-300 md:flex items-center ${right !== 0 ? ' flex-row-reverse translate-x-[510px] md:opacity-0' : 'md:opacity-100 translate-x-[0px]'}`}>
                        <div className={`${right !== 0 ? 'ml-20' : 'md:mr-[193px]'}`}>
