        ...
        <Box className="flex w-screen max-md:pb-[210px] z-50 md:overflow-hidden">
            <div className='flex relative'>
                <SignBox className={`md:flex md:h-[1278px] max-md:pt-[300px] relative w-screen items-center justify-center`}
                    ...
                    <div className={`max-md:hidden absolute ${right === 0 ? 'WhySIGN opacity-100' : 'ABOUT opacity-100'} transition-all duration-1000 opacity-0 top-0 left-0 w-full h-full`}></div>
                    <div className={`md:flex relative md:w-[1200px] w-screen items-center `}>
                        <SignCharacteristicBox className={`${right !== 0 ? ' translate-x-[908px]' : 'translate-x-[0px]'} max-md:pl-[68px] max-md:pb-[72px] max-md:pr-[77px] max-md:pt-[135px] transition-all duration-300`}>
                            <div >
                                <div className='max-md:flex justify-between'>
                                    <p className=' font-light text-[56px] max-md:text-[24px] leading-[120%]' onClick={() => setMobileRight(0)}>{`${right !== 0 ? right === 1 ? '关于我们' : '发展历程' : '公司特色'}`}</p>
                                ...
                                <p className={`font-extralight max-md:hidden font-[Lexend] text-[22px] ${right !== 0 ? right === 1 ? 'tracking-[15.18px]' : 'tracking-[2.2px]' : 'tracking-[0.53em]'} mt-4 opacity-60 uppercase leading-[160%]`}>{`${right !== 0 ? right === 1 ? 'ABOUT US' : 'Development Path' : 'Why SIGN?'}`}</p>
                            </div>
                        </SignCharacteristicBox>
                        <div className={`transition-all duration-300 md:flex items-center ${right !== 0 ? ' flex-row-reverse translate-x-[510px] md:opacity-0' : 'md:opacity-100 translate-x-[0px]'}`}>
                            <div className={`md:ml-[202px]`}>
                                {mobileRight === 0 ? <div className='md:h-[488px] max-md:px-6 overflow-hidden'>
                                    <div className={`transition-all duration-300 ${characteristicType === 1 ? 'translate-y-[-488px]' : characteristicType === 2 ? 'translate-y-[-976px]' : characteristicType === 3 ? 'translate-y-[-1464px]' : ''} `}>
                                        <div className='md:h-[488px] md:w-[680px] max-md:mt-11 flex flex-col justify-center'>
                                            <div className='flex'>
                                                <p className='text-[40px] mr-[26px] max-md:mr-[16px]  font-normal leading-[120%] max-md:text-[22px] text-[#FF4B00]'>资深导师团队</p>
                                                <Image className='md:hidden max-md:w-[94px] max-md:h-6' src={signCharacteristic_title} alt='' />
                                                <TitleBeam type={`${characteristicType === 0 ? 'open' : 'down'}`} />
                                            </div>
                                            <div className={`md:w-[488px] ${characteristicType === 0 ? 'opacity-100' : 'md:opacity-0'} mt-12`}>
                                        ...
                                        <div className={`md:h-[488px] md:w-[680px] max-md:mt-20 flex ${characteristicType === 1 ? 'opacity-100' : 'md:opacity-0'} flex-col justify-center`}>
                                            <div className='flex '>
                                                <p className='text-[40px] mr-[26px] max-md:mr-[16px] font-normal leading-[120%] max-md:text-[22px] text-[#FF4B00]'>独家顶级设备配置</p>
                                                <Image className='md:hidden max-md:w-[94px] max-md:h-6' src={signCharacteristic_title} alt='' />
                                                <TitleBeam type={`${characteristicType === 1 ? 'open' : 'down'}`} />
                                            </div>
                                        ...
                                        <div className={`md:h-[488px] md:w-[680px] ${characteristicType === 2 ? 'opacity-100' : 'md:opacity-0'} max-md:mt-20 flex flex-col justify-center`}>
                                            <div className='flex'>
                                                <p className='text-[40px] mr-[26px] max-md:mr-[16px] font-normal leading-[120%] max-md:text-[22px] text-[#FF4B00]'>高质量高效率精致教学</p>
                                                <Image className='md:hidden max-md:w-[94px] max-md:h-6' src={signCharacteristic_title} alt='' />
                                                <TitleBeam type={`${characteristicType === 2 ? 'open' : 'down'}`} />
                                            </div>
                                            <p className='text-[14px] max-md:hidden md:w-[488px] mt-12 font-light leading-[320%] opacity-70'>
                                                免费专业入学评估<br />
                                                一对一课程定制+未来职业规划<br />
                                        ...
                                        <div className={`md:h-[488px] ${characteristicType === 3 ? 'opacity-100' : 'md:opacity-0'} md:w-[680px] max-md:mt-20 flex flex-col justify-center`}>
                                            <div className='flex'>
                                                <p className='text-[40px] mr-[26px] max-md:mr-[16px] font-normal leading-[120%] max-md:text-[22px] text-[#FF4B00]'>产学结合&背景提升</p>
                                                <Image className='md:hidden max-md:w-[94px] max-md:h-6' src={signCharacteristic_title} alt='' />
                                                <TitleBeam type={`${characteristicType === 3 ? 'open' : 'down'}`} />
                                            </div>
                        ...
                        <div className={` transition-all duration-500 ${right === 0 ? 'left-[-650px] opacity-0' : 'left-[120px] opacity-100'} absolute left-[-650px] max-md:hidden mt-12`}>
                            <div className=" flex items-center">
                                <div className={`h-[457px] max-md:hidden overflow-hidden`}>
                                    <div
                                        className={` transition-all duration-300 ${right === 2 ? 'translate-y-[-457px]' : 'translate-y-[0px]'} `} >
                                ...
                                <div className='ml-20 h-[420px] overflow-hidden w-[599px]'>
                                    <div className={` transition-all duration-500 ${right === 2 ? ' translate-y-[-420px]' : 'translate-y-[0px]'}`}>
                                        <div className={`h-[420px] ${right === 2 ? 'opacity-0' : 'opacity-100'}`}>
                                        ...
                                        <div className={`h-[420px] ${right === 1 ? 'opacity-0' : 'opacity-100'} flex justify-center flex-col`}>
                                            <p className='text-[14px] font-light leading-[220%] opacity-70'>
                                                2012年 初始音乐团队创办SOUNDPIC工作室
                                            </p>
                                            <p className='text-[14px] font-light leading-[320%] opacity-70'>
                                                2016年 万像循声文化传媒有限公司正式成立 ｜总部位于杭州滨江，并在南京、重庆设立分棚
                                            </p>
                                            <p className='text-[14px] font-light leading-[320%] opacity-70'>
                        ...
                        <div className={`h-[457px] ${right === 0 ? '' : 'md:hidden'} max-md:hidden overflow-hidden`}>
                            <div
